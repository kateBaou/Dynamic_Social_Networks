...
def read_pickled_metrics(u,v):
	...
	with open('RGGmetrics_brownian/ranked_degree/ranked_degrees'+str(u)+'_'+str(v)+'.pickle', 'rb') as f:
		order_degrees = pickle.load(f)

	with open('RGGmetrics_brownian/ranked_nodes/ranked_nodes'+str(u)+'_'+str(v)+'.pickle', 'rb') as f:
...
for u in range(10):
	for v in range(1,4):
		...
		plt.ylim(5,50)
		plt.savefig('RGGplots_brownian/rank_plots/rank'+str(u)+'/ranked'+str(u)+'_'+str(v)+'_0'+'.eps', format='eps', dpi=1000)
		...
		for i in range(1,len(ranked_nodes),49):
			...
			plt.ylim(5,50)
			name='RGGplots_brownian/rank_plots/rank'+str(u)+'/ranked'+str(u)+'_'+str(v)+'_'+str(i)+'.eps'
