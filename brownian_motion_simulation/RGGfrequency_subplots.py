...
def read_pickled_metrics(i,j):
	...
	with open('RGGmetrics_brownian/density/density'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
		density = pickle.load(f)

	with open('RGGmetrics_brownian/clustering/clustering'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
		clustering = pickle.load(f)

	with open('RGGmetrics_brownian/assortativity/assortativity'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
		assortativity = pickle.load(f)

	with open('RGGmetrics_brownian/impact/impact'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
		impact = pickle.load(f)

	with open('RGGmetrics_brownian/degree/degree'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
		degree = pickle.load(f)

	with open('RGGmetrics_brownian/neigh_degree/neigh_degree'+str(i)+'_'+str(j)+'.pickle', 'rb') as f:
...
for i in range(10):
	for j in range(1,4):
		...
		fig = pl.gcf()
		fig.suptitle("Frequency vs RGG Metrics", fontsize=30)

		plt.savefig('RGGplots_brownian/frequency_plots/frequency'+str(i)+'_'+str(j)+'.eps', format='eps', dpi=1000)
